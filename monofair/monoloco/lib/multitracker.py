...
class JDETracker(object):
    def __init__(self, opt, frame_rate=15):
        self.arch = 'dla_34'
        self.heads = {'hm': 1, 'wh': 4, 'id': 128, 'reg': 2}
        self.head_conv=256
        self.load_model = './modelsx/fairmot_dla34.pth'
        ###################################################
        self.conf_thresh = 0.3
        self.track_buffer=400 #200
        ###################################################
        self.K = 500
        self.meanx = [0.408, 0.447, 0.47]
        self.stdx = [0.289, 0.274, 0.278]
        ...
        self.frame_id = 0
        self.det_thresh = self.conf_thresh
        self.buffer_size = int(frame_rate / 30.0 * self.track_buffer)
        self.max_time_lost = self.buffer_size
        self.max_per_image = self.K
        self.mean = np.array(self.meanx, dtype=np.float32).reshape(1, 1, 3)
        self.std = np.array(self.stdx, dtype=np.float32).reshape(1, 1, 3)
    ...
    def update(self, im_blob, img0):
        ...
        remain_inds = dets[:, 4] > self.conf_thresh
