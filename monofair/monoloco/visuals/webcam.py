...
def upload_blob(bucket_name, source_file_name, destination_blob_name):
    ...
    print("Logger: SAVED VIDEO TO CLOUD!") 
    print(
    ...
    for camera_addr in itertools.cycle(camera_ip_address):
        ...
        while True:
            ...
            if cur_time - prev_time < 4:
                ...
                resized_frame, _, _, _ = letterbox(frame)

                if camera_addr not in buffer.keys():
                    buffer.update({camera_addr : []})
 
                if len(buffer[camera_addr]) < 240:
                    buffer[camera_addr].append(resized_frame)
                else:
                    buffer[camera_addr] = buffer[camera_addr][60:]
                    buffer[camera_addr].append(resized_frame)
                ...
                if pifpaf_out:
                    ...
                    for ilv in range(0, len(non_violations)):
                        ...
                        if max_iou > 0.30:
                            ...
                            if unique_id not in unique_reid:
                                ...
                                try:
                                    #cv2.imwrite("/home/lauretta/lauretta_server_python/social-dist-1/person_imgs/" + str(unique_id) + ".jpg", imagex[int(box_y1):int(box_y2), int(box_x1):int(box_x2)])
                                    if age is not None:
                                        age_temp = age
                                    else:
                                        age_temp = 99
                    ...
                    for ilv in range(0, len(violations)):
                        ...
                        try:
                            ...
                            file_name = save_to_video(buffer[camera_addr])
                            try:
                            ...
                            PARAMS = {'property_id':int(property_id) ,'cam_id': int(camera_id), 'created_at':curr_time, 
                            'floor_id': int(floor_id), 'incident_type_id': incident_type_id, 'cam_view_x1': box_x1, 
                            'cam_view_y1': box_y1, 'cam_view_x2': box_x2, 'cam_view_y2': box_y2, 
                            'floorplan_x1': floor_x1, 'floorplan_y1': floor_y1, 'floorplan_x2' : floor_x2, 
                            'floorplan_y2': floor_y2, 'total_people': len(violations), 'footage_path': url_link, 
                            'status_id': status_id, 'person_ids': person_buffer, 'distance': distance  }
                        ...
                        try:
                            PARAMS = {'property_id':int(property_id),'cam_id': int(camera_id), 'floor_id': int(floor_id), 
                            'person_instance_x1': box_x1, 'person_instance_y1': box_y1, 'person_instance_x2': box_x2, 
                            'person_instance_y2': box_y2, 'floorplan_x1': floor_x1, 'floorplan_y1': floor_y1, 
                            'floorplan_x2': floor_x2, 'floorplan_y2' : floor_y2, 'direction': direction_degrees}
                            print(PARAMS)
                            q = requests.post(url = PERSON_INSTANCE_ENDPOINT, json = PARAMS, headers=HEADER)
                            person_instance_id = q.json()['data']['id']
                        ...
                        if max_iou > 0.30: 
                            ...
                            if unique_id not in unique_reid:
                                ...
                                try:
                                    ...
                                    if (((timeslice_now - timeslice_start_time).total_seconds()) >= 60):
                                        timeslice_start_time = datetime.datetime.now()
                                        try:
                                            file_name = save_to_video(buffer[camera_addr])
                                            url_link = upload_blob(
                                            ...
                                            try:
                                                ## property_id, zone_id, incident_type_id, video_link, start_time
                                                ## start_time, end_time,incident_type_id,    person_instance_id,    video_link,
                                                PARAMS = {'property_id':int(property_id),'start_time': datetime.datetime.now(), 'end_time': datetime.datetime.now(),
                                                          'incident_type_id': 1, 'video_link': url_link, 
                                                          'floor_plan x': floor_x1, 'floorplan_y': floor_y1, 'floor_id':int(floor_id)}
                                                # PARAMS = {'start_time': datetime.datetime.now(), 'end_time': datetime.datetime.now(), 'incident_type_id': 5, 'person_instance_id': int(person_instance_id), 'video_link': url_link }
                                                #r = requests.post(url = INCIDENT_PERSON_INSTANCE_ENDPOINT, json = PARAMS, headers=HEADER)
                                ...
                                if (((timeslice_now - timeslice_start_time).total_seconds()) >= 300):
                                    ...
                                    interval_id = str(timeslice_now.strftime("%Y%m%d")) + "_" + str(interval)
                                    try:
                                        PARAMS = {'property_id':int(property_id) ,'intervalID': interval_id, 'timestamp': timeslice_now, 'PersonID': person_temp_id, 'iou': '1', 'floorplan_x1': floor_x1, 'floorplan_y1':floor_y1, 'floor_id': int(floor_id), 'camera_id': int(camera_id), 'angle':direction_degrees, 'incident_group':incident_group, 'incident_numpeople': distance_count}
    ...
    for idx, _ in enumerate(dic_out['xyz_pred']):
        ...
        if sdist_bool :
            if len(bboxes) > 0:
                ...
                violations.append([idx,int(camera_id),int(floor_id),int(incident_type_id),box_x1, box_y1, box_x2, box_y2, floor_x1, floor_y1, floor_x2, floor_y2, status_id, direction_degrees, distance_count])
            ...
            floor_x1, floor_y1, floor_x2, floor_y2 = get_floor_cords(xz_centers, angles[idx], idx)
            direction_degrees = math.degrees(angles[idx])
            non_violations.append([idx,int(camera_id),int(floor_id),int(incident_type_id),box_x1, box_y1, box_x2, box_y2, floor_x1, floor_y1, floor_x2, floor_y2, status_id, direction_degrees])
